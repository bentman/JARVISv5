# JARVISv5 Merged Development Roadmap (now on M5S)
...
## ~~Milestone 4 — Tool System + Sandboxed Execution~~ completed
...
- System Information Tools (Milestone 4 Extended: `get_timestamp`, `get_system_info`, `get_working_directory`)
...
Useful for context (Milestone 4 Extended):
...
## ~~Milestone 5 — Privacy & Security Controls~~ partial, at-rest encryption deferred
...
- Privacy-aware tool wrapper / gating for external calls
- At-rest encryption for conversation/memory persistence (**Deferred / Investigation Required**)
- Deferred (post-M5 security): Tier 3 Code Execution tools — `execute_python`, `execute_shell`, and expanded filesystem access.
...
> More detail regarding conceptual design and architecture guidance found in `roadmap-20260224-PrivacySecurity.md` 

**Project.md citations:** §8.1, §8.2, §6.3

---

## ~~Milestone 6 — Implement Redis cache for retrieval/context results (with traceability)~~ partial, health status deferred
...
## ~~Milestone 7 — Hybrid Semantic Retrieval~~ completed
...
- Future/deferred: Tier 5 Memory Operations tools (`search_memory`, `add_knowledge`) are intentionally out of current milestone scope and tied to later retrieval/memory evolution.
...
## ~~Milestone 8 — Search + Policy-Bound Escalation~~ mostly complete (needs fine tuning)
...
### Deliverables
- Tier 4 Web/Search tool surface: search provider integration (`search_web`) and URL fetch capability (`fetch_url`)
...
## ~~Milestone 9 — UI Completion (Spec-Aligned)~~ mostly complete, deferred SSE Endpoint
